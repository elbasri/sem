...
<?php } ?>  
		<?php echo $this->Form->create('Vacance',array('action'=>'recherche'));?>
		...
		<td style="text-align:left;">Search Start date of leave</td>
		</tr>
		...
		<td style="text-align:left;">Search by Number of leave</td>
		</tr>
...
<div class="affichage" style="float:right;width:80%"> 
<?php } ?> 
	<h1>Search result in the leaves:</h1>
	<table >
	<tr>
	<th>ID</th>
	<th>Due of leave</th>
	...
	<td><?php if(date('Y-m-d')>$post['Vacance']['datef']) echo "<font color='red'>leave Done</font>"; else if(date('Y-m-d')<=$post['Vacance']['datef'] && date('Y-m-d')>=$post['Vacance']['dated'])echo "<font color='green'>Current leave</font>"; else if (date('Y-m-d')<$post['Vacance']['dated'])echo "<font color='blue'>Upcoming leave</font>";?></td>
	<td><?php echo $this->Html->link($post['Vacance']['employe_nom'],array('controller'=>'employes','action'=>'view',$post['Vacance']['employe_id'],Inflector::slug($post['Vacance']['employe_nom'],$replacement ='_')));?></td>
	<td>
	<?php 
