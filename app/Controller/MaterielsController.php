...
class MaterielsController extends AppController{
	...
	public $uses = array('Materiel','Piece','Commande','Client','Stockcategorie','Stockaction','Evennement');
	public $paginate= array('limit'=>'10','order'=>array('Materiel.nom'=>'asc'));
	public $paginateasc= array('limit'=>'10','order'=>array('Materiel.nom'=>'asc'));
	public $paginatedesc= array('limit'=>'10','order'=>array('Materiel.nom'=>'desc'));
	...
	if($this->Session->read('typearticle')=='t'){
		...
		$bureau=$this->Materiel->find('all',array('conditions'=>array('typemateriel'=>'Matériel Mobilier de bureau','qte > 0'),'order'=>array('nom'=>'asc')));
		$informatique=$this->Materiel->find('all',array('conditions'=>array('typemateriel'=>'Matériel informatique'),'order'=>array('nom'=>'asc')));
		$medicoHospitalier=$this->Materiel->find('all',array('conditions'=>array('typemateriel'=>'Matériel medicoHospitalier','qte > 0'),'order'=>array('nom'=>'asc')));
		$medicoTechnique=$this->Materiel->find('all',array('conditions'=>array('typemateriel'=>'Matériel medicoTechnique','qte > 0'),'order'=>array('nom'=>'asc')));
		$exploitation=$this->Materiel->find('all',array('conditions'=>array('typemateriel'=>"Matériel d'exploitation",'qte > 0'),'order'=>array('nom'=>'asc')));
...
$name=$this->request->params['controller']."_liste_".md5(time());
	...
	$this->layout = 'admin';
		...
		if($titre==$titredetest){
			...
			if($post){
				...
				$this->set('post',$post);
				echo $post['Materiel']['typemateriel'];
				}
				else
	...
	$this->set(compact('options','optionsfo','optionsc','optionsi','optionsm','optionsfa','optionsfb'));
		if($this->request->is('Post')){
			...
			*/
			...
			$lastid=$this->Materiel->getLastInsertId();
			$lastpost=$this->Materiel->findById($lastid);
			$this->set('lastid',$lastpost['Materiel']['ref']+1);
				$this->Stockaction->create();
				$this->request->data['Stockaction']['materiel_id']=$this->Materiel->getInsertID();
				$this->request->data['Stockaction']['materiel_nom']=$this->request->data['Materiel']['nom'];
				$this->request->data['Stockaction']['nom']="entrée";
				$this->request->data['Stockaction']['fournisseur_id']=$this->request->data['Materiel']['fournisseur_id'];
				$this->request->data['Stockaction']['fournisseur_nom']=$this->request->data['Materiel']['fournisseur_nom'];
				$this->request->data['Stockaction']['client_nom'] = "Dépot Al Fida Mers Sultan";
                            if($this->request->data['Materiel']['ref']!=null){
				$this->request->data['Stockaction']['ref'] = $this->request->data['Materiel']['ref'];
				}
				$this->request->data['Stockaction']['typevaleur']=$this->request->data['Materiel']['type'];
				$this->request->data['Stockaction']['qte']=$this->request->data['Materiel']['qte'];
				$this->request->data['Stockaction']['date']=$this->request->data['Materiel']['date'];
				$this->request->data['Stockaction']['user_id']=$this->Auth->User('id');
				$this->Stockaction->save($this->request->data);
	...
	$this->layout = 'admin';
		...
		if($this->request->is('Post'))
		{
			...
			$des=$this->request->data['Materiel']['serie'];
				//$realisation=$this->Materiel->find('all',array('conditions'=>array('Materiel.ref="'.$this->request->data['Materiel']['serie'].'"')));
				$realisation=$this->Materiel->find('all',array('conditions'=>array('OR' => array('Materiel.ref LIKE' => "%$des%"))));
				}
