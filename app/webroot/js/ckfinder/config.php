...
if(isset($_SESSION['Auth']['User'])){
	if($_SESSION['Auth']['User']['role']=="admin" || $_SESSION['Auth']['User']['role']=="moderateur")
		return true ;//!empty($_SESSION['Auth']['User']);
	else
		return false;
	}
else
...
echo $baseUrl = substr($fullpath, $docrootlen) ;//substr($fullpath, $docrootlen);
if(substr($baseUrl, 0, 13)=="/app/webroot/")
			$baseUrl = substr($baseUrl, 12);
/*
...
$config['ResourceType'][] = Array(
		...
		'maxSize' => 0,
		'allowedExtensions' => 'avi,bmp,csv,doc,docx,fla,flv,gif,jpeg,jpg,mid,mov,mp3,mp4,mpeg,mpg,ods,odt,pdf,png,ppt,pptx,txt,xls,xlsx',
		'deniedExtensions' => '');
